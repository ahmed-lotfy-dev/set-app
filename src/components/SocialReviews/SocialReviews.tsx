...
export default function SocialReviews({}: Props) {
  ...
  const chunks = useMemo(() => {
    return Array.from(
      { length: Math.ceil(reviews.length / chunkSize) },
      (_, i) => reviews.slice(i * chunkSize, i * chunkSize + chunkSize),
    );
  }, [chunkSize]);
  ...
  return (
    <section className="w-full bg-section-bg text-text-dark pt-10 pb-20">
      <div className="max-w-360 m-auto px-6 relative">
        {/* Social Heading Area */}
        <div className="flex justify-between flex-1 flex-col md:flex-row items-center text-center md:text-start mt-24.75 pb-10 md:px-2.75">
          <h2 className="text-[48px] font-semibold leading-14.5 tracking-tight max-w-125">
            Setapp in your words.
          </h2>

          <div className="flex items-center flex-col md:flex-row gap-10">
            <p className="text-[18px] font-normal leading-8 text-text-dark/60 max-w-75 flex-1">
        ...
        <div className="flex justify-end items-center gap-7.5 z-10 mt-6.25 mb-5">
          <button
            className="cursor-pointer hover:opacity-50 transition-opacity"
            onClick={prevSlide}
          >
            <img src={carouselArrowLeft} alt="Previous" className="w-3.5 h-6" />
          </button>
          <button
            className="cursor-pointer hover:opacity-50 transition-opacity"
            onClick={nextSlide}
          >
            <img src={carouselArrowRight} alt="Next" className="w-3.5 h-6" />
          </button>
        </div>

        <div className="overflow-hidden w-full mt-6.25">
          <div
          ...
          >
            {chunks.map((chunk, chunkIndex) => (
              <div
                key={chunkIndex}
                className={`min-w-full grid gap-6 ${isDesktop ? "grid-cols-3" : "grid-cols-1"}`}
              >
        ...
        {/* Pagination Dots */}
        <div className="flex justify-center items-center gap-6.25 px-5.75 mt-6 mb-6.25">
          {chunks.map((_, index) => (
            <button
              key={index}
